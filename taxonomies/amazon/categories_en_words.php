<?php

$categories = [
    'Arts & Crafts' => [
        'Beading & Jewelry Making'          => [
            'Beading Supplies'                   => [
                'Bead Looms',
                'Beading Cords & Threads',
                'Beading Kits',
                'Beading Mats, Trays & Boards',
                'Beading Needles',
                'Fuse & Perler Beads',
            ],
            'Beads & Bead Assortments',
            'Charms',
            'Engraving Machines & Tools',
            'Fusible Glass Supplies',
            'Jewelry Casting Supplies'           => [
                'Casting Machines',
                'Casting Tools',
                'Metal Casting Molds',
                'Resin Casting Molds',
            ],
            'Jewelry Findings'                   => [
                'Cord Ends',
                'Earring Backs & Findings',
                'End Caps',
                'Eye Pins',
                'Head Pins',
                'Jewelry Clasps',
                'Jewelry Making Chains',
                'Jump Rings',
                'Pin Backs',
                'Wire',
            ],
            'Jewelry Making Display & Packaging Supplies',
            'Jewelry Making Kits',
            'Jewelry Making Tools & Accessories' => [
                'Diamond & Gold Testers'     => [
                    'Diamond Testers',
                    'Gold Testers',
                ],
                'Jewelry Hammers',
                'Jewelry Pliers',
                'Jewelry Sizers & Mandrels',
                'Kilns & Firing Accessories' => [
                    'Kilns',
                    'Firing Accessories',
                ],
                'Loupes',
            ],
            'Jewelry Patterns',
            'Metal Stamping Tools',
            'Polishing & Buffing',
            'Purse Making',
            'Storage',
            'Wax Molding Materials',
        ],
        'Crafting'                          => [
            'Basket Making',
            'Candle Making'                  => [
                'Dyes',
                'Kits',
                'Molds',
                'Scents',
                'Wax',
                'Wicks',
            ],
            'Ceramics & Pottery'             => [
                'Ceramic & Pottery Tools',
                'Clay Extruders, Mixers & Presses',
                'Clay Molds',
                'Clays & Doughs'             => [
                    'Clay',
                    'Dough',
                ],
                'Glazes',
                'Kilns & Firing Accessories' => [
                    'Kilns',
                    'Firing Accessories',
                ],
                'Pottery Wheels & Accessories',
                'Pug Mills & Slab Rollers',
                'Ready-to-Paint Ceramics',
            ],
            'Craft Supplies'                 => [
                'Adhesives'     => [
                    'Adhesive Removers',
                    'Adhesive Sheets & Sprays' => [
                        'Adhesive Sheets',
                        'Adhesive Sprays',
                    ],
                    'Adhesive Waxing',
                    'Art Glues & Pastes',
                    'Arts & Crafts Tape',
                    'Craft Glue Guns & Sticks' => [
                        'Glue Guns',
                        'Glue Gun Sticks',
                    ],
                    'Fabric Adhesives',
                    'Laminating Supplies',
                    'Mounting Boards',
                ],
                'Brads',
                'Clock Kits',
                'Craft Bells',
                'Craft Foam',
                'Craft Sticks',
                'Cutting Tools' => [
                    'Art Knives & Blades',
                    'Craft Shears',
                    'Cutting Mats',
                    'Glass Cutting Tools',
                    'Mat Cutters & Blades',
                    'Paper Trimmers' => [
                        'Rotary Trimmers',
                        'Stack & Guillotine Trimmers',
                        'Trimmer Blades',
                    ],
                    'Rotary Cutters',
                    'Scissors',
                ],
                'Face Painting',
                'Feathers & Boas',
                'Glitter',
                'Gold & Metal Leaf',
                'Macrame & Knotting',
                'Pipe Cleaners',
                'Wiggle Eyes',
            ],
            'Doll Making',
            'Fabric Ribbons',
            'Floral Arranging'               => [
                'Floral Foam',
                'Floral Moss',
                'Floral Picks',
                'Floral Tapes & Wraps',
                'Frogs & Kenzans',
                'Vase Fillers',
            ],
            'Leathercraft'                   => [
                'Lacing Needles',
                'Leather Cord & Lacing',
                'Leather Strips, Shapes, & Scraps',
                'Leathercraft Accessories',
                'Leathercraft Rivets',
                'Stamping & Punching' => [
                    'Punching',
                    'Stamping',
                ],
            ],
            'Mosaic Making',
            'Paper & Paper Crafts'           => [
                'Bookbinding',
                'Card Making Kits',
                'Decoupage',
                'Embossing'                         => [
                    'Accessories',
                    'Embossers',
                    'Embossing Folders',
                    'Tools & Tool Sets',
                ],
                'Paper'                             => [
                    'Art Tissue & Crepe Paper' => [
                        'Art Tissue',
                        'Crepe Paper',
                    ],
                    'Card Stock',
                    'Cellophane Wrap',
                    'Construction Paper',
                    'Decorative Paper',
                    'Kraft Paper',
                    'Metallic Paper & Foil',
                    'Multimedia Surfaces',
                    'Origami Paper',
                    'Paper Ribbon & Raffia',
                    'Parchment',
                    'Printmaking Paper',
                    'Transfer Paper',
                ],
                'Paper Craft Tools',
                'Papermaking',
                'Papier-Mâché',
                'Punches',
                'Quilling'                          => [
                    'Kits',
                    'Quilling Strips',
                    'Tools',
                ],
                'Scissors, Trimmers & Shears'       => [
                    'Rotary Trimmers',
                    'Stack & Guillotine Trimmers',
                    'Trimmer Blades',
                ],
                'Stencils, Templates & Accessories' => [
                    'Stencils & Templates',
                    'Stencil Brushes & Pouncers',
                    'Stencil Ink',
                ],
            ],
            'Picture Framing'                => [
                'Frame Molding',
                'Frame Rulers & Straight Edges',
                'Frame Sections & Parts',
                'Framing Tools',
                'Mat Cutters & Blades',
                'Matting & Mounting Materials',
                'Picture Hanging Materials',
            ],
            'Scratchboards & Foil Engraving' => [
                'Foil Engraving',
                'Scratchboards',
            ],
            'Sculpture Supplies'             => [
                'Clay',
                'Modeling Compounds',
                'Modeling Tools',
                'Molding & Casting',
                'Release Agents',
                'Wire & Armatures',
            ],
            'Soap Making'                    => [
                'Bases & Melts',
                'Dyes',
                'Molds',
                'Scents',
            ],
            'Stained Glass Making'           => [
                'Glass Sheets',
                'Lead & Foil',
                'Tools',
            ],
            'Suncatcher Supplies',
            'Weaving & Spinning'             => [
                'Ball Winders',
                'Spinning Wheels',
                'Weaving Loom Tools & Accessories',
                'Weaving Looms',
                'Wool Roving',
            ],
            'Woodcrafts'                     => [
                'Unfinished Wood',
                'Wood Burning Tools',
                'Wood Carving Tools',
            ],
        ],
        'Fabric',
        'Fabric Decorating'                 => [
            'Application Tools',
            'Dyes',
            'Fabric & Textile Paints',
            'Fabric Decorating Kits',
            'Fixatives',
        ],
        'Gift Wrapping Supplies'            => [
            'Enclosure Cards',
            'Gift Bags',
            'Gift Boxes',
            'Gift Wrap Bows',
            'Gift Wrap Cellophane',
            'Gift Wrap Cellophane Bags',
            'Gift Wrap Crinkle & Filler Paper',
            'Gift Wrap Paper',
            'Gift Wrap Ribbons',
            'Gift Wrap Tags',
            'Wrapping Sets',
            'Wrapping Tissue',
        ],
        'Knitting & Crochet'                => [
            'Ball Winders',
            'Crochet Hooks',
            'Crochet Kits',
            'Crochet Patterns',
            'Crochet Thread',
            'Knitting & Crochet Notions',
            'Knitting Kits',
            'Knitting Looms & Boards',
            'Knitting Needles',
            'Knitting Patterns',
            'Needle Cases',
            'Yarn',
            'Yarn Storage',
        ],
        'Needlework'                        => [
            'Cross-Stitch'             => [
                'Aida Cloth',
                'Counted Kits',
                'Floss',
                'Patterns',
                'Stamped Kits',
            ],
            'Embroidery'               => [
                'Floss',
                'Hoops',
                'Kits',
                'Storage',
            ],
            'Felt Applique Kits',
            'Latch Hook'               => [
                'Latch Hook Kits',
                'Latch Hook Supplies',
            ],
            'Needle Felting Supplies'  => [
                'Kits',
                'Needles',
                'Tools',
                'Wool Roving',
            ],
            'Needlepoint'              => [
                'Blank Canvas',
                'Kits',
                'Patterned Canvas',
                'Patterns',
                'Yarn Needles',
            ],
            'Punch Needle & Rug Punch' => [
                'Punch Needle Supplies',
                'Rug Punch Supplies',
            ],
            'Tatting & Lacemaking',
        ],
        'Organization, Storage & Transport' => [
            'Art & Poster Tubes',
            'Art Tool & Sketch Boxes',
            'Beading Storage',
            'Craft & Sewing Supplies Storage' => [
                'Embroidery Storage',
                'Scrapbooking Storage',
                'Sewing Storage',
                'Yarn Storage',
            ],
            'Drying & Print Racks',
            'Flat & Vertical Files',
            'Paint Brush Organizers & Holders',
            'Pen, Pencil & Marker Cases',
            'Portfolios',
            'Storage Boxes & Organizers',
            'Storage Cabinets',
        ],
        'Painting, Drawing & Art Supplies'  => [
            'Art Paper'       => [
                'Artist Trading Cards',
                'Bristol Paper & Vellum' => [
                    'Bristol Paper',
                    'Vellum',
                ],
                'Drawing Paper',
                'Easel Pads',
                'Mixed Media Paper',
                'Newsprint Paper',
                'Palette Paper',
                'Pastel Paper',
                'Sketchbooks & Notebooks',
                'Tracing Paper',
                'Watercolor Paper',
            ],
            'Boards & Canvas' => [
                'Canvas Boards & Panels',
                'Canvas Pads',
                'Canvas Tools & Accessories',
                'Clayboard',
                'Gessoboard',
                'Hardboard',
                'Pre-Stretched Canvas',
                'Rolled Canvas',
                'Wood Art Boards',
            ],
            'Drawing'         => [
                'Art Blenders',
                'Art Sets',
                'Artists\' Manikins',
                'Calligraphy & Sumi Brushes',
                'Drawing & Lettering Aids',
                'Drawing Media' => [
                    'Chalk',
                    'Charcoals',
                    'Crayons',
                    'Inks',
                    'Markers',
                    'Pastels',
                    'Pencils',
                    'Pens',
                ],
                'Drawing Tables & Boards',
                'Erasers',
                'Fixatives',
                'Light Boxes',
                'Nibs',
                'Rubbing Plates & Supplies',
                'Sharpeners',
            ],
            'Easels',
            'Painting'        => [
                'Airbrush Materials'            => [
                    'Airbrush Sets',
                    'Spray Booths',
                ],
                'Kits',
                'Mixing Trays',
                'Paint Finishes',
                'Paint Making Materials',
                'Paint Mediums & Additives',
                'Paint Pens, Markers & Daubers' => [
                    'Daubers',
                    'Pens & Markers',
                ],
                'Paint Primers & Sealers',
                'Paint Sponges',
                'Paint Thinners & Solvents',
                'Paint-By-Number Kits'          => [
                    'Adults\' Paint-By-Number Kits',
                    'Children\'s Paint-By-Number Kits',
                ],
                'Paintbrushes'                  => [
                    'Angled Paintbrushes',
                    'Bright Paintbrushes',
                    'Fan Paintbrushes',
                    'Filbert Paintbrushes',
                    'Foam Paintbrushes',
                    'Hake Paintbrushes',
                    'Highliner Paintbrushes',
                    'Letterer Paintbrushes',
                    'Mop Paintbrushes',
                    'One-Stroke Paintbrushes',
                    'Oval-Wash Paintbrushes',
                    'Paintbrush Sets',
                    'Pointed-Round Paintbrushes',
                    'Quill Paintbrushes',
                    'Round Paintbrushes',
                    'Script Paintbrushes',
                    'Square-Wash Paintbrushes',
                ],
                'Paints',
                'Palette Knives',
                'Palettes & Palette Cups'       => [
                    'Palettes',
                    'Palette Cups',
                ],
            ],
            'Brush & Pen Cleaners',
        ],
        'Party Decorations & Supplies'      => [
            'Aisle Runners',
            'Balloons',
            'Banners & Garlands',
            'Card Boxes',
            'Cardboard Cutouts',
            'Centerpieces',
            'Confetti',
            'Guestbooks',
            'Luminarias',
            'Sky Lanterns',
            'Streamers',
            'Tablecovers',
            'Tissue Pom Poms',
        ],
        'Printmaking'                       => [
            'Etching Supplies'                  => [
                'Etching Accessories',
                'Etching Materials',
                'Etching Tools',
            ],
            'Heat Press Machines & Accessories' => [
                'Heat Press Machines',
                'Parts & Accessories',
            ],
            'Printing Presses & Accessories',
            'Printmaking Inks',
            'Relief & Block Printing Materials' => [
                'Brayers',
                'Linoleum Cutters',
                'Printing Linoleum',
            ],
            'Screen Printing'                   => [
                'Accessories',
                'Screen Printing Kits',
                'Squeegees',
            ],
        ],
        'Scrapbooking & Stamping'           => [
            'Adhesive Vinyl',
            'Adhesives'                   => [
                'Glue Guns & Sticks' => [
                    'Glue Guns',
                    'Glue Gun Sticks',
                ],
                'Glues & Pastes',
                'Photo Mounting Corners',
                'Tape',
            ],
            'Albums & Refills'            => [
                'Albums',
                'Photo Albums' => [
                    'Acid-Free & Archival Page Albums',
                    'Bookshelf Albums',
                    'Magnetic Page Albums',
                    'Three-Ring Binder Albums',
                    'Wedding Albums',
                    'Photo Album Accessories',
                ],
                'Refills',
            ],
            'Chipboard',
            'Cutting Mats',
            'Die-Cutting & Embossing'     => [
                'Die-Cut Cartridges',
                'Die-Cut Machines',
                'Die-Cuts',
                'Embossing Machines & Folders' => [
                    'Accessories',
                    'Embossers',
                    'Embossing Folders',
                    'Tools & Tool Sets',
                ],
                'Replacement Blades',
                'Tools & Accessories',
            ],
            'Kits',
            'Paper & Card Stock'          => [
                'Card Stock',
                'Paper',
            ],
            'Paper Punches',
            'Pens & Markers'              => [
                'Markers',
                'Pens',
            ],
            'Photo Transfer & Coloring Supplies',
            'Scrapbooking Embellishments' => [
                'Brads',
                'Embellishments',
                'Glitter',
                'Quilling Strips',
            ],
            'Scrapbooking Tools',
            'Stamps & Ink Pads'           => [
                'Ink Pads',
                'Stamp Blocks',
                'Stamps' => [
                    'Clear Stamps',
                    'Cling Stamps',
                    'Wood Stamps',
                ],
                'Texture Plates',
            ],
            'Stickers & Sticker Machines',
            'Storage',
            'Stencils & Templates'        => [
                'Stencils & Templates',
                'Stencil Brushes & Pouncers',
                'Stencil Ink',
            ],
        ],
        'Sewing'                            => [
            'Embroidery Machines',
            'Industrial Machines',
            'Quilting'                           => [
                'Batting',
                'Cutting Mats',
                'Frames',
                'Hoops',
                'Notions',
                'Patterns',
                'Pre-Cut Quilt Squares',
                'Quilting Fabric Assortments',
                'Quilting Pins',
                'Rotary Cutter Blades',
                'Rotary Cutters',
                'Rulers & Ruler Racks',
                'Stencils & Templates' => [
                    'Stencils',
                    'Templates',
                ],
            ],
            'Serger & Overlock Machine Accessories',
            'Sergers & Overlock Machines',
            'Sewing Machine Parts & Accessories' => [
                'Attachments',
                'Carrying Cases',
                'Oil',
                'Parts',
                'Presser Feet',
            ],
            'Sewing Machines',
            'Sewing Notions & Supplies'          => [
                'Bobbins',
                'Dress Forms & Mannequins',
                'Elastic',
                'Fasteners'           => [
                    'Buckles',
                    'Buttons',
                    'Cord Locks',
                    'Eyelets & Grommets',
                    'Interlocking Tape',
                    'Snaps',
                    'Undergarment Sewing',
                    'Zippers',
                ],
                'Hand Sewing Needles' => [
                    'Beading',
                    'Embroidery & Crewel',
                    'Quilting',
                    'Sharps',
                    'Yarn & Tapestry',
                ],
                'Heat Transfer Film',
                'Heat Transfer Paper',
                'Interfacing',
                'Machine Needles'     => [
                    'Embroidery Machine Needles',
                    'Quilting Machine Needles',
                    'Serger Needles',
                    'Sewing Machine Needles',
                ],
                'Pillow Forms & Foam' => [
                    'Cushion & Upholstery Foam',
                    'Pillow Forms',
                ],
                'Pins & Pincushions'  => [
                    'Pins',
                    'Pincushions',
                ],
                'Sewing Tools'        => [
                    'Marking & Tracing Tools',
                    'Pinking Shears',
                    'Repair Patches',
                    'Scissors',
                    'Seam Rippers',
                    'Tailors Awl',
                    'Tape Measures & Rulers' => [
                        'Rulers',
                        'Tape Measures',
                    ],
                    'Thimbles',
                    'Threaders',
                ],
                'Stabilizers',
                'Stuffing & Polyester Fill',
                'Tapes & Adhesives'   => [
                    'Bias Tape',
                    'Bias Tape Makers',
                    'Fabric Adhesives',
                    'Fusible & Hem Tape',
                ],
            ],
            'Sewing Patterns & Templates',
            'Sewing Project Kits',
            'Storage & Furniture'                => [
                'Sewing Baskets',
                'Sewing Cabinets',
                'Storage',
            ],
            'Thread & Floss'                     => [
                'Embroidery Thread & Floss' => [
                    'Embroidery Machine Thread',
                    'Floss',
                ],
                'Quilting',
                'Serger',
                'Sewing',
            ],
            'Trim & Embellishments'              => [
                'Appliques & Decorative Patches' => [
                    'Appliques',
                    'Decorative Patches',
                ],
                'Beaded Trim',
                'Bias Tape',
                'Braids & Cords' => [
                    'Braids',
                    'Cords'
                ],
                'Fabric Stud & Gem Setters',
                'Fringe Trim',
                'Iron-on Transfers',
                'Lace',
                'Piping',
                'Rhinestones & Sequins',
                'Ribbons',
                'Rick Rack',
                'Sequin Trim',
                'Tassels',
            ],
        ],
    ],
    'Automotive'    => [],
];
